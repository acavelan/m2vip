...
int main(int argc, char **argv)
{
	...
	//cudaMallocHost((void**)&image, size * 3 * sizeof(unsigned char), cudaHostAllocMapped);
	cudaMallocHost((void**)&image, size * 3 * sizeof(unsigned char));
	memcpy(image, ILimage, size * 3 * sizeof(unsigned char));
	...
	unsigned char *d_image, *d_grayimg = 0;
	...
	CUCHECK(cudaMalloc((void**)&d_image, size * 3 * sizeof(unsigned char)));
    ...
    for(int i=0; i<nStreams; i++)
    {
		...
		CUCHECK(cudaMemcpyAsync(&d_image[offset * 3], &image[offset * 3], streamSize * 3 * sizeof(unsigned char), cudaMemcpyHostToDevice, streams[i]));
	}
	...
	CUCHECK(cudaFree(d_image));
	CUCHECK(cudaFreeHost(image));
