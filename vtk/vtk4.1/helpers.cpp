...
void GetMemorySize(const char *description)
{
    ...
    cerr  << helpers_prefix << "Memory: for \"" << description << "\", the program has requested " <<size<<  unit<< endl;
    ...
    //melin
    int count = fscanf(file, "%u%u%u%u%u%u%u", &size, &rss, &share, &trs, &lrs, &drs, &dt);
    ...
    float fact=getpagesize()/1024.0;
    string unit="Ko";

    fclose(file);
    cerr  << helpers_prefix << "Memory: for \"" << description << "\", the program has requested " <<drs*fact<<  unit<< endl;
#endif
}
