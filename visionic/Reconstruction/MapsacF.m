...
function [f,f_sq_errors, n_inliers,inlier_index] = torr_mapsac_F(x1,y1,x2,y2, no_matches, m3, no_samp)
...
%    [no_F big_result]= torr_F_constrained_fit(tx1,ty1,tx2,ty2,m3);
    ...
    for j = 1:no_F
        ...
        %calcul de T !!!!! ALBOUY
        %estimation de sigma
        sigma = 1.4826*(1.0+5.0/(no_matches-7.0))*sqrt(median(et));
        T = 5.99*sigma^2;
        %fin ALBOUY
...
%calcul de T !!!!! ALBOUY
%estimation de sigma
sigma = 1.4826*(1.0+5.0/(no_matches-7.0))*sqrt(median(f_sq_errors));
T = 5.99*sigma^2;
%fin ALBOUY
